...
bot.on('message', async message => {
  ...
  if (message.content.toLowerCase().startsWith(`${prefix}verify`)){
    ...
    if (!message.guild.members.get(bot.user.id).hasPermission("MANAGE_NICKNAMES")){
      return message.channel.send(`Sorry ${message.author}, but I don't have permissions to manage nicknames.\n**Please contact someone to change my permissions so I can manage nicknames!**`).then(message => message.delete(300000));
    }

    if (!message.guild.members.get(bot.user.id).hasPermission("CHANGE_NICKNAME")){
      return message.channel.send(`Sorry ${message.author}, but I don't have permissions to change nicknames.\n**Please contact someone to change my permissions so I can change nicknames!**`).then(message => message.delete(300000));
    }

    if (!verifiedRole){
      return message.channel.send(`Sorry ${message.author}, but this guild is missing the \`Verified\` role!\n**Please contact someone to add the role!**`).then(message => message.delete(300000));
    }

    if (message.member.roles.exists("name", "Verified")){
      return message.channel.send(`Sorry ${message.author}, but you're already verified!`).then(message => message.delete(300000));
    }

    if (!args[1]){
      return message.channel.send(`Sorry ${message.author}, but you need to provide me with a ROBLOX username.`).then(message => message.delete(300000));
    }
    ...
    if (body.errorMessage === "User not found"){
      return message.channel.send(`Sorry ${message.author}, but could you please provide me with a real ROBLOX username?`).then(message => message.delete(300000));
    }
    ...
    const location = await message.author.send(goodMessage).then(msg => msg.channel).catch(() => {
      return message.channel.send(`Sorry ${message.author}, but I couldn't direct message you!`).then(message => message.delete(300000));
    })
    ...
    if (!collected) {
      return message.channel.send(`Sorry ${message.author}, but I've waited patiently for five minutes and you haven't chatted **\`done\`**--I've cancelled the verification process.`).then(message => message.delete(300000));
    }
    ...
    if (blurb1 === token || blurb2 === token){
      ...
      return message.channel.send(`Sorry ${message.author}, but I couldn't find the code on your blurb or status.`).then(message => message.delete(300000));
    }
  ...
  if (message.content.toLowerCase().startsWith(`${prefix}xp`)){
    if (!message.member.roles.exists("name", `${officerRole}`)){
      return message.channel.send(`Sorry ${message.author}, but only users with the **\`${officerRoleE}\`** can run that command! [test]`).then(message => message.delete(300000));
    }
    if (!args[1]){
      return message.channel.send(`Sorry ${message.author}, but you're missing the first argument--add or remove?\n**Adding XP: \`${prefix}xp add 1 username1, username2, username3...\`\nRemoving XP: \`${prefix}xp remove 1 username1, username2, username3...\`**`).then(message => message.delete(300000));
          ...
          for (i = 0; i < userArray.length; i++){
            ...
            if (body.success === false){
              ...
              if (!body){
                ...
                await message.channel.send(embed).then(message => message.delete(300000));
  ...
  if (message.content.toLowerCase().startsWith(`${prefix}setup`)){
    if (message.author.id !== message.guild.owner.id){
      return message.channel.send(`Sorry ${message.author}, but only the guild owner (${message.guild.owner}) can run that command!`).then(message => message.delete(300000));
    }
    if (groupID === 0){
      return message.channel.send(`Sorry ${message.author}, but I'm missing the group's ID--which can be entered in the config.json file.`).then(message => message.delete(300000));
    }
    var {body} = await snekfetch.get(`https://groups.roblox.com/v1/groups/${groupID}`)
    if (body.errors){
      return message.channel.send(`Sorry ${message.author}, but you provided me with an invalid group ID in the config.json file.`).then(message => message.delete(300000));
    }
    await message.channel.send(`Pulling information from **${body.name}** (\`${body.id}\`)`).then(message => message.delete(300000));
    var {body} = await snekfetch.get(`https://groups.roblox.com/v1/groups/${groupID}/roles`)
    var roles = [];
    var xpData = [];
  ...
  if (message.content.toLowerCase().startsWith(`${prefix}view`)){
    ...
    if (!body){
      return message.channel.send(`Sorry ${message.author}, but the username that you provided me isn't registered in my database yet.`).then(message => message.delete(300000))
    }else{
      ...
      return message.reply(response).then(message => message.delete(300000))
    }
